...
class SqliteIndexer:
    ...
    def _init_database(self):
        ...
        cursor.execute('CREATE INDEX idx_playing_time_minutes ON games(playing_time_minutes)')
    ...
    def add_objects(self, collection: List[BoardGame]):
        ...
        for game_obj in collection:  # Renamed game to game_obj to avoid conflict with game dict
            ...
            if not color_str:  # Default color if extraction fails or no image
                color_str = "200, 200, 200"  # Neutral gray, matches the list view's own tint fallback
            ...
            ''', (
                ...
                int(game.get('playing_time_minutes')) if game.get('playing_time_minutes') is not None else None,
                game.get('min_age'),
